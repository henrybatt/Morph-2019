...
class Tssp{
    public:
        ...
        uint16_t values[TSSP_NUM];
        uint16_t sortedValues[TSSP_NUM];
        uint8_t indexes[TSSP_NUM];
        uint16_t tsspCounter;

        bool ballVisible;
        

      private:
        uint16_t tempValues[TSSP_NUM];

        uint16_t angle;
        uint16_t strength;
        // double scaledSin[TSSP_NUM];
        // double scaledCos[TSSP_NUM];
